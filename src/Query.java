public class Query {
    ...
    public static LinkedList<Integer> andQuery(String Query){
        ...
        boolean found = invertedIndex.searchWordInInvertedIndex(terms[0].trim().toLowerCase());
        if(found){
            A= invertedIndex.invertedIndex.retrieve().docIDS;
        }
        for(int i = 1; i< terms.length; i++){
            found = invertedIndex.searchWordInInvertedIndex(terms[i].trim().toLowerCase());
            if(found){
                B= invertedIndex.invertedIndex.retrieve().docIDS;
            }
            A=andQuery(A,B);//
        }
        return A;
